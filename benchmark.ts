...
class MockAudioBuffer {
  ...
  constructor(length: number, numberOfChannels: number) {
    this.length = length;
...
function originalGenerateStereoWaveformData(audioBuffer: MockAudioBuffer) {
  const width = 800;
...
function optimizedGenerateStereoWaveformData(audioBuffer: MockAudioBuffer) {
  const width = 800;
  const samples = audioBuffer.length;
  const samplesPerPixel = Math.floor(samples / width);
