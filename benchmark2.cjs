...
function optimizedBranchingOuterGenerateStereoWaveformData(audioBuffer) {
  const width = 800;
  const samples = audioBuffer.length;
  const samplesPerPixel = Math.floor(samples / width);
...
function runBenchmark() {
  ...
  console.log("Running Branching Outer Stereo...");
  start = performance.now();
  for (let i = 0; i < iterations; i++) {
    optimizedBranchingOuterGenerateStereoWaveformData(bufferStereo);
  }
  let optimizedBranchingOuterStereoTime = performance.now() - start;
  ...
  console.log("Running Branching Outer Mono...");
  start = performance.now();
  for (let i = 0; i < iterations; i++) {
    optimizedBranchingOuterGenerateStereoWaveformData(bufferMono);
  }
  let optimizedBranchingOuterMonoTime = performance.now() - start;


  console.log(`\nResults (Total time for ${iterations} iterations):`);
  console.log(`Stereo Original: ${originalStereoTime.toFixed(2)}ms`);
  console.log(`Stereo Branching Outer: ${optimizedBranchingOuterStereoTime.toFixed(2)}ms (${((originalStereoTime - optimizedBranchingOuterStereoTime)/originalStereoTime*100).toFixed(2)}% faster)`);

  console.log(`Mono Original: ${originalMonoTime.toFixed(2)}ms`);
  console.log(`Mono Branching Outer: ${optimizedBranchingOuterMonoTime.toFixed(2)}ms (${((originalMonoTime - optimizedBranchingOuterMonoTime)/originalMonoTime*100).toFixed(2)}% faster)`);
